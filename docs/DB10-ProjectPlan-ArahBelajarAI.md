...
**ArahBelajarAI** hadir untuk menjembatani gap ini dengan memberikan analisis berbasis data nyata sehingga pengguna tidak lagi "*menebak-nebak*" dalam perjalanan belajar mereka.
...
2. Bagaimana sistem rekomendasi dapat menghasilkan jalur belajar (*learning path*) yang paling efisien berdasarkan tingkat urgensi skill dan ketersediaan kursus?

**ArahBelajarAI** adalah aplikasi PWA (Progressive Web Apps) yang dirancang untuk mempercepat kesiapan kerja talenta digital Indonesia. Menggunakan teknologi *Machine Learning* khususnya NLP (*Natural Language Processing*), platform ini menganalisis profil pengguna terhadap ribuan data lowongan kerja aktif untuk mengidentifikasi *skill gap*. Hasil akhirnya adalah skor kesiapan kerja dan peta jalan belajar (*roadmap*) yang dikurasi secara otomatis. Fokus MVP adalah memberikan nilai instan pada fitur analisis dan rekomendasi, sementara fitur pendukung seperti pendamping belajar (*AI Companion*) akan dikembangkan pada fase berikutnya.
...
* **Eksklusi (Phase 2):** Fitur Pomodoro Timer dan *AI Companion* untuk menemani progres belajar.
...
| **Minggu 2** (16-22 Feb) | **AI & Backend Dev** | • Implementasi **all-MiniLM-L6-v2** & *ChromaDB*. • Pembuatan API Endpoint (*User Assessment*, *Skill Extractor,* *Skill Gap Analysis & Generate Learning Path*). • *Milestone: Backend dapat menampilkan hasil skill extraction, skill gap, dan generate learning path.* |
| **Minggu 3** (23 Feb-1 Mar) | **Frontend & Integration** | • Pembuatan UI dan Component untuk page: *User Assessment, Job Selector, Skill Extractor, dan Dashboard Hasil Analisis*. • Visualisasi hasil learning path (*Timeline*). • Milestone: *Integrasi Frontend ke Backend API berjalan dengan lancar dan error di handle dengan baik*. |
| **Minggu 4** (2-8 Mar) | **Testing & Refinement** | • *Mid-Checkpoint Report* . • Re-check Offline support. • User Testing (*Internal*) memastikan tidak ada error terjadi. • Optimasi kecepatan response (\< 3 detik). • Polish UI/UX agar user flow lebih baik. |
...
1. **Bahasa Pemrograman:**  
   ...
   * **TypeScript:** Untuk pengembangan Frontend yang *type-safe* dan meminimalisir bug.  
2. **Framework & Libraries:**  
   ...
   * **Sentence Transformers (*all-MiniLM-L6-v2)*:** Model NLP ringan namun akurat untuk mengubah teks menjadi vektor (embeddings).  
   * **Chroma DB (*Vector DB*):** Library untuk pencarian vektor yang sangat cepat dan efisien.  
...
4. **Data:**  
   ...
   * Job Datasets:  
     ...
     4. [FreelanceIncomeSkill Datasets](https://www.kaggle.com/datasets/shaistashahid/freelancer-income-vs-skills)  
...
| :---- | :---- | :---- |
| **Kapasitas Solo Developer** | Kewalahan mengerjakan Frontend & Backend. | Menggunakan AI Assistant (*Claude Code*) untuk otomatisasi pembuatan komponen UI dan boilerplate. |
| **Kualitas Dataset Rendah** | Rekomendasi kursus tidak relevan atau *outdated*. | Melakukan *data cleaning* ketat di awal dan memprioritaskan dataset kursus dengan rating tinggi (\>4.0) dan update terbaru. |
| **Scope Creep** | Fitur melebar (misal: ingin menambahkan fitur login dan integration LLM) sehingga MVP tidak selesai. | Disiplin pada "*Guest Mode First*". Fitur login dan penyimpanan permanen hanya akan dikerjakan jika core feature sudah 100% stabil (Masuk ke fase 2). |
